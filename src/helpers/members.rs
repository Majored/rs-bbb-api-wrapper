...
impl<'a> MembersHelper<'a> {
    pub async fn fetch_self(&self) -> Result<MemberData> {
        self.wrapper.get(&format!("{}/members/self", crate::BASE_URL)).await
    }

    pub async fn fetch_by_id(&self, member_id: u64) -> Result<MemberData> {
        self.wrapper.get(&format!("{}/members/{}", crate::BASE_URL, member_id)).await
    }

    pub async fn fetch_by_name(&self, member_name: &str) -> Result<MemberData> {
        self.wrapper.get(&format!("{}/members/username/{}", crate::BASE_URL, member_name)).await
    }

    pub async fn list_recent_bans(&self) -> Result<BanData> {
        self.wrapper.get(&format!("{}/members/bans", crate::BASE_URL)).await
    }

    pub async fn list_profile_posts(&self) -> Result<Vec<ProfilePostData>> {
        self.wrapper.get(&format!("{}/members/profile-posts", crate::BASE_URL)).await
    }

    pub async fn fetch_profile_post(&self, profile_post_id: u64) -> Result<ProfilePostData> {
        self.wrapper.get(&format!("{}/members/profile-posts/{}", crate::BASE_URL, profile_post_id)).await
    }
